...
ModelLoader::loadModel(const std::filesystem::path &path) {
  ...
  if (j.contains("elements")) {
    for (const auto &elemJson : j["elements"]) {
      ...
      // Rotation - Support VS-style element-level rotationX/Y/Z
      // Format 1: "rotationY": -45.0, "rotationOrigin": [0, 0, 0]
      // Format 2: "rotation": {"axis": "y", "angle": -45, "origin": [0, 0, 0]}
      bool hasElementRotation = elemJson.contains("rotationX") ||
                                elemJson.contains("rotationY") ||
                                elemJson.contains("rotationZ");

      if (hasElementRotation) {
        elem.hasRotation = true;

        // Parse origin
        if (elemJson.contains("rotationOrigin")) {
          std::vector<float> origin = elemJson["rotationOrigin"];
          elem.rotation.origin =
              glm::vec3(origin[0], origin[1], origin[2]) / 16.0f;
        } else {
          elem.rotation.origin = glm::vec3(0.5f, 0.5f, 0.5f); // Default center
        }

        // Parse rotation angle
        float rx = elemJson.value("rotationX", 0.0f);
        float ry = elemJson.value("rotationY", 0.0f);
        float rz = elemJson.value("rotationZ", 0.0f);
      ...
      } else if (elemJson.contains("rotation")) {
        elem.hasRotation = true;
