...
BlockDef::BlockDefinition BlockLoader::parseJSON(const nlohmann::json &j) {
  ...
  if (j.contains("code")) {
    ...
    // Enforce 'lithos' namespace
    if (def.code.find(':') == std::string::npos) {
      def.code = "lithos:" + def.code;
    }
  }
...
BlockLoader::createBlockFromDefinition(const BlockDef::BlockDefinition &def,
  ...
  // Enforce 'lithos' namespace for block registration
  std::string registerCode = variantCode;
  if (registerCode.find(':') == std::string::npos) {
    registerCode = "lithos:" + registerCode;
  }

  block =
      BlockFactory::getInstance().createBlock(className, finalId, registerCode);
...
bool BlockLoader::matchesPattern(const std::string &pattern,
  ...
  // Handle namespace mismatch: if pattern has no namespace but value does,
  // strip value namespace
  std::string valueToCheck = value;
  if (pattern.find(':') == std::string::npos &&
      valueToCheck.compare(0, 7, "lithos:") == 0) {
    valueToCheck = valueToCheck.substr(7);
  }

  size_t starPos = pattern.find('*');
  if (starPos == std::string::npos) {
    return pattern == valueToCheck;
  }

  std::string prefix = pattern.substr(0, starPos);
  std::string suffix = pattern.substr(starPos + 1);

  if (valueToCheck.length() < prefix.length() + suffix.length()) {
    return false;
  }

  bool prefixMatch = valueToCheck.compare(0, prefix.length(), prefix) == 0;
  bool suffixMatch =
      valueToCheck.compare(valueToCheck.length() - suffix.length(),
