...
SlabBlock::SlabBlock(block_id id, const std::string &name) : Block(id, name) {}
...
bool SlabBlock::isOpaque() const { return false; }

// But we want it to be solid for collision (when collision is implemented)
bool SlabBlock::isSolid() const { return true; }

Block::RenderShape SlabBlock::getRenderShape() const {
