...
uint8_t World::getLight(int x, int y, int z)
{
    int cx = floorDiv(x, CHUNK_SIZE);
    int cy = floorDiv(y, CHUNK_SIZE);
    int cz = floorDiv(z, CHUNK_SIZE);
    ...
    if(!c) return 15; // Assume bright if outside world (or 0?) - Skylight is 15.
...
void World::setBlock(int x, int y, int z, BlockType type)
{
    ...
    if(c) { 
        ...
        // Force update neighbors if on edge
        if(lx == 0) { Chunk* n = getChunk(cx-1, cy, cz); if(n) n->updateMesh(); }
        if(lx == CHUNK_SIZE-1) { Chunk* n = getChunk(cx+1, cy, cz); if(n) n->updateMesh(); }
        
        if(ly == 0) { Chunk* n = getChunk(cx, cy-1, cz); if(n) n->updateMesh(); }
        if(ly == CHUNK_SIZE-1) { Chunk* n = getChunk(cx, cy+1, cz); if(n) n->updateMesh(); }
        
        if(lz == 0) { Chunk* n = getChunk(cx, cy, cz-1); if(n) n->updateMesh(); }
        if(lz == CHUNK_SIZE-1) { Chunk* n = getChunk(cx, cy, cz+1); if(n) n->updateMesh(); }
    }
}
